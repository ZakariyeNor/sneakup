...
class TestWebhookView:
    ...
    def test_missing_signature_header(self):
        request = self.factory.post(self.url, data=b'{}', content_type='application/json')
    ...
    def test_invalid_payload_raises_value_error(self, mock_construct_event):
        ...
        request = self.factory.post(self.url, data=b'invalid', content_type='application/json',
                                    HTTP_STRIPE_SIGNATURE='sig')
        response = webhook.webhook(request)
        assert response.status_code == 400

    @patch('checkout.webhook.stripe.Webhook.construct_event')
    def test_invalid_signature_raises_signature_verification_error(self, mock_construct_event):
    ...
    def test_valid_event_calls_correct_handler(self, mock_handler_class, mock_construct_event):
        # Prepare mock event
        ...
        mock_handler.handle_payment_intent_succeeded.return_value = HttpResponse('Success', status=200)
        mock_handler.handle_event.return_value = HttpResponse('Generic event', status=200)
        ...
        response = webhook.webhook(request)
        mock_handler.handle_payment_intent_succeeded.assert_called_once_with(event)
        assert response.status_code == 200
        assert response.content == b'Success'

        # Change event type to unknown
        mock_construct_event.return_value = {'type': 'unknown.event'}
        mock_handler.handle_event.return_value = HttpResponse('Handled generic', status=200)
