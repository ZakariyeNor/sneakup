...
class Order(models.Model):
    ...
    order_number = models.UUIDField(
            max_length=15, default=uuid.uuid4, editable=False, unique=True)
    ...
    def update_total(self):
        ...
        if self.order_total < settings.FREE_DELIVERY:
            self.delivery = (
                self.order_total * settings.DELIVERY_PERCENTAGE / Decimal(100)
        ...
        self.vat = (
            self.order_total * vat_rate
            ).quantize(Decimal('0.01'), ROUND_HALF_UP)

        # Calculte the grand total and round up
        self.grand_total = (
            self.order_total + self.delivery + self.vat
            ).quantize(Decimal('0.01'), ROUND_HALF_UP)
        self.save()
...
class OrderLineItem(models.Model):
    ...
    def save(self, *args, **kwargs):
        """
        Calculate and set the line item total based
        on the product price and quantity
    ...
    def __str__(self):
        ...
        to identify the specific order item
        clearly in logs and admin interfaces.
        """
