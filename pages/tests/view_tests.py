...
def test_contact_view_post_valid(client):
    ...
    assert any(
        "Your message has been sent successfully!" in str(m) for m in messages)
...
def test_about_view(client):
    hero = AboutPageHero.objects.create(
        overlay_title='Hero', hero_image='test.jpg')
    mission = OurMission.objects.create(
        our_mission_title='Mission', our_mission_description='Desc')
    materials = OurMaterials.objects.create(
        our_materials_title='Materials', our_materials_description='Desc')
    best_selling = BestSelling.objects.create(
        best_selling_title='Best Seller',
        best_description='Desc', best_image='best.jpg')

    for i in range(5):
        NewArrivals.objects.create(
            new_name=f'New {i}',
            launched_date='2023-01-0' + str(i+1), new_image='new.jpg')
        LaunchedProducts.objects.create(
            launched_name=f'Launched {i}',
            launched_date='2023-01-0' + str(i+1), launched_image='launch.jpg')
